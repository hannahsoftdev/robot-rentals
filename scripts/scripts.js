...
function recalculateCost() {
    ...
    if (selectedModel === "XYZ") {
        cost = rentalDuration * 100;
    } else if (selectedModel === "CPRG") {
        cost = rentalDuration * 213;
    }
...
switchModelButton.addEventListener("click", toggleModel);
...
function updateDuration() {
    ...
    // Ensure newDuration is a number
    newDuration = Number(newDuration);

    if (!isNaN(newDuration) && newDuration >= 0) {
        rentalDuration = newDuration;
        durationTextElement.innerHTML = rentalDuration;
        recalculateCost();
    } else {
...
changeDurationButton.addEventListener("click", updateDuration);
